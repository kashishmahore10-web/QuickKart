...
const ProductCard = ({product}: Props) => {
    ...
    const currency = (import.meta as any).env?.VITE_CURRENCY_SYMBOL || "$";
    const {addToCart} = useCart()
    const navigate = useNavigate()
    return (
        <div className='bg-white rounded-2xl overflow-hidden shadow hover:shadow-md
        transition-all duration-300 grouo animate-fade-in cursor-pointer'
        onClick={()=> navigate(`/products/${product._id}`)}>
            ...
            <div className='relative aspect-square overflow-hidden'>
                ...
                <div className='absolute top-3 left-3 flex flex-wrap gap-1.5' >
                    ...
                    uppercase bg-app-orange text-white rounded-full'>{product.discount}% OFF</span>}
            ...
            <div className='p-3.5 text-zinc-700'>
                ...
                <div className='flex items-center justify-between'>
                    <div className='flex items-center gap-2 truncate'>
                        <span className='text-base font-medium'>{currency}
