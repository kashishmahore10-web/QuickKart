import { ArrowRightIcon, BikeIcon, ChevronDownIcon, LogOutIcon, MapPinIcon, MenuIcon, PackageIcon, Search, ShieldIcon, ShoppingCartIcon, UserIcon, XIcon } from 'lucide-react';
...
const Navbar = () => {
    ...
    const handleSearch = (e: React.SubmitEvent)=>{
        e.preventDefault()
        if(searchQuery.trim()){
            navigate('/serch?q=${encodeURIComponent(searchQuery.trim())}')
            setSearchQuery("")
        }
    ...
    justify-between h-16 gap-4'>
                {/* Logo */}
                <Link to="/" className='flex items-center gap-2 text-[22px] font-medium shrink-8 '>
                    <BikeIcon size={24} />
                    Quickkart</Link>
        ...
        text-zinc-600'>
                    ...
                    {/* Search Bar */}
                    <form onSubmit={handleSearch} className='hidden sm:flex flex-1vmax-w-sm text-xs
        sm:text-sm'>
                                    ...
                                    <div className='absolute right-0 mt-2.5 w-56 bg-white rounded-xl shadow-lg border border-app-border py-2 z-50 animate-fade-in'>
                                        ...
                                        <div className='px-4 py-2 flex flex-col gap-2'>
                                            ...
                                            <Link to='/products' className='dropdown-link'>
                                                <ArrowRightIcon size={16} /> Products
                                            </Link>
                                            <Link to='/deals' className='dropdown-link'>
                                                <ArrowRightIcon size={16} /> Deals
                                            </Link>
                                            {user?.isAdmin && (
                                            ...
                                            { user && (
                                                <div className='border-t border-app-border pt-1'>
                                                    ...
                                                    text-sm text-app-error hover:bg-red-50 -full transition-colors'>
